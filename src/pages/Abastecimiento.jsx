...
const Abastecimiento = () => {
  ...
  const showPesadaForm = (row) => {
    const pesadasAcumuladas = [];
    let pollingInterval = null;
    let isManualFocus = false; // Flag para pausar la balanza cuando el usuario escribe
    ...
    Swal.fire({
      ...
      didOpen: () => {
        ...
        // Listeners para detectar cuando el usuario quiere escribir manualmente
        pesadaInput.addEventListener('focus', () => { isManualFocus = true; });
        pesadaInput.addEventListener('blur', () => { isManualFocus = false; });

        pollingInterval = setInterval(async () => {
          // Si el usuario tiene el cursor en el input, NO sobreescribimos con la balanza
          ...
          try {
            ...
            if (pesadaInput && !isManualFocus) pesadaInput.placeholder = "Error Conexión";
          }
        }, 1200);
