...
const EditarOperacion = () => {
    ...
    const handleCierreClick = async () => {
        ...
        if (!inspeccionData?.header?.finalRevisado) {
            ...
            return; // ⛔ ESTO SÍ BLOQUEA (inspección no aprobada)
        }

        // ✅ VALIDACIÓN 2: Solo validar calidad y saldo, NO tolerancia individual
        const validacion = await validarCierreVB();
        
        // ⚠️ CAMBIO CLAVE: Si hay errores de tolerancia, SOLO mostrar warning, NO bloquear
        if (!validacion.valido) {
            const mensajeCompleto = validacion.mensajes.join('\n');
            
            // Si es por FALTA DICTAMEN DE CALIDAD, sí bloquear
            if (mensajeCompleto.includes('FALTA DICTAMEN DE CALIDAD')) {
                await Swal.fire({
                    title: 'Advertencia',
                    html: `<div style="text-align: left; white-space: pre-wrap; font-family: monospace;">${mensajeCompleto}</div>`,
                    icon: 'warning',
                ...
                return; // ⛔ BLOQUEAR por falta de dictamen
            }
            
            // Si es por FUERA TOLERANCIA, mostrar warning pero PERMITIR continuar
            if (mensajeCompleto.includes('Fuera Tolerancia')) {
                await Swal.fire({
                    title: 'Advertencia',
                    html: `<div style="text-align: left; white-space: pre-wrap; font-family: monospace;">${mensajeCompleto}</div>`,
                    icon: 'warning',
                ...
                // ✅ NO RETORNAR - PERMITIR CONTINUAR AL CIERRE
            }
        }

        // ✅ Si pasó todas las validaciones (o solo tuvo warnings de tolerancia), mostrar confirmación
        const result = await Swal.fire({
            title: '¿Confirmar Cierre?',
            text: 'Se CERRARA la operación.',
            icon: 'question',
