...
//         setModalLoading(true);
//         try {
//             // Obtenemos el usuario del contexto de autenticación o localStorage
...
//                 usuario: userObj.nombre // 👈 Pasamos el usuario real
//             });
...
//                                     totalCal: 0,
//                                     totalAtados: balance.atadosSobrante || 0, // <--- Valor real desde el balance
//                                     totalRollos: balance.rollosSobrante || 0,  // <--- Valor real desde el balance
//                                     bSobrante: true,           // ✅
//                                     bScrap: false,             // ✅
...
//                                     totalCal: 0,
//                                     totalAtados: balance.atadosScrapSeriado || 0, // <--- Valor real
//                                     totalRollos: 0 
//                                 },
//                                 { 
...
//                                     totalCal: 0,
//                                     totalAtados: balance.atadosScrapNoSeriado || 0, // <--- Valor real
//                                     totalRollos: 0 
//                                 }
//                             ].map((item) => {
//                                 return (
                                ...
                                return (
                                    <div
                                        ...
                                        onClick={isOperationEditable ? async () => {
                                            ...
                                            // ✅ CORRECCIÓN PRINCIPAL: Manejo de Scrap Seriado
                                            if (item.tipo === 'Scrap Seriado') {
                                                try {
                                                    const mermaRes = await axiosInstance.get(`/registracion/pesaje/codigo-merma/${operacionId}`);
                                                    const CodigoProductoS = mermaRes.data.CodigoProductoS || '';
                                                    
                                                    // ✅ CAMBIO: No mostrar error si el código está vacío, solo continuar con fallback
                                                    if (!CodigoProductoS) {
                                                        console.warn("⚠️ No hay código de merma para scrap, usando código original como fallback");
                                                    }
                                                    ...
                                                    lineaDataParaModal = {
                                                        ...
                                                        CodSerie: header.SerieLote,
                                                        CodigoProducto: header.CodigoProducto || '',
                                                        CodigoProductoS: CodigoProductoS, // Puede estar vacío
                                                        LoteID: header.LoteID,
                                                        Tarea: item.tipo,
                                                        Maquina: header.maquinaId,
                                                        NroBatch: header.Batch,
                                                    ...
                                                    // ✅ Solo mostrar error si es un error de red real (no 404 por código vacío)
                                                    if (err.response?.status !== 404) {
                                                        Swal.fire('Error', 'No se pudo cargar el código de merma.', 'error');
                                                        console.error(err);
                                                    } else {
                                                        // ✅ Fallback: continuar con código vacío si es 404
                                                        console.warn("⚠️ Endpoint devolvió 404, continuando con código vacío");
                                                        lineaDataParaModal = {
                                                            tipoEspecial: item.tipo,
                                                            bSobrante: false,
                                                            bScrap: true,
                                                            ...
                                                            CodSerie: header.SerieLote,
                                                            CodigoProducto: header.CodigoProducto || '',
                                                            CodigoProductoS: '', // Fallback vacío
                                                            LoteID: header.LoteID,
                                                            Tarea: item.tipo,
                                                            Maquina: header.maquinaId,
                                                            NroBatch: header.Batch,
                                                ...
                                                // Para Sobrante y Scrap No Seriado
                                                lineaDataParaModal = {
                                                    tipoEspecial: item.tipo,
                                                    bSobrante: item.tipo === 'Sobrante',
                                                    bScrap: item.tipo.includes('Scrap'),
