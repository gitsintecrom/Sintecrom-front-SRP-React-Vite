// // src/pages/OperacionesSlitter.jsx
...
// export default OperacionesSlitter;
...
// import { useAuth } from '../context/AuthContext';
// import maquinas from '../data/maquinas.json';
// import './OperacionesSlitter.css';

// // Configuración del tema (SIN modificar font-size)
// createTheme('adminLteDark', {
//     text: { primary: '#FFFFFF', secondary: 'rgba(255, 255, 255, 0.7)' },
//     background: { default: '#343a40' },
//     divider: { default: '#454d55' },
...
//                     await fetchOperaciones();
//                 } catch (error) {
//                     console.error('Error al registrar la pesada:', error);
//                     Swal.fire('Error', error.response?.data?.error || 'No se pudo registrar la pesada.', 'error');
//                 }
//             }
//         });
...
//     // ✅ ESTILOS CONDICIONALES SIMPLIFICADOS (SIN CSS complejo)
//     const conditionalRowStyles = [
//         {
//             when: row => row.status === 'BLOQUEADA',
//             style: { 
...
//                 color: 'white',
//                 cursor: 'pointer' // ✅ Solo cursor pointer, sin efectos CSS complejos
//             }
//         },
//         {
//             when: row => row.status === 'EN_CALIDAD' || row.status === 'CALIDAD_DICTAMINADA' || row.status === 'TOLERANCIA_EXCEDIDA',
...
//                 color: 'black',
//                 cursor: 'pointer' // ✅ Solo cursor pointer
//             }
//         },
//         {
//             when: row => row.status === 'SUSPENDIDA',
...
//     // ✅ TOOLTIP NATIVO HTML (funciona en todos los navegadores)
//     const getRowTooltip = (row) => {
//         const editableStatuses = ['EN_PROCESO', 'EN_CALIDAD', 'CALIDAD_DICTAMINADA', 'TOLERANCIA_EXCEDIDA'];
//         if (editableStatuses.includes(row.status)) {
//             return 'Clic para editar operación';
//         }
//         if (row.status === 'LISTA' || row.status === 'SUSPENDIDA') {
//             return hasDiscrepancy(row.Stock, row.Kilos_Balanza) 
//                 ? 'Clic para registrar pesada (hay discrepancia)' 
//                 : 'Operación no editable en este estado';
//         }
//         if (row.status === 'BLOQUEADA') {
//             return 'Operación bloqueada - complete los requisitos previos';
//         }
//         return '';
//     };

//     // ✅ CUSTOM STYLES SIMPLIFICADOS (SIN font-size, SIN pseudo-elementos)
//     const customStyles = {
//         rows: {
//             style: row => ({
//                 cursor: ['EN_PROCESO', 'EN_CALIDAD', 'CALIDAD_DICTAMINADA', 'TOLERANCIA_EXCEDIDA'].includes(row.status) 
//                     ? 'pointer' 
//                     : 'default',
//                 title: getRowTooltip(row) // ✅ Tooltip nativo HTML
//                 // ✅ SIN font-size para mantener el tamaño original
//                 // ✅ SIN transition/filter para evitar conflictos
//             })
//         }
//     };
...
// ✅ COMPONENTE TOOLTIP PERSONALIZADO (para react-data-table-component)
const RowTooltip = ({ visible, message, position }) => {
...
const OperacionesSlitter = () => {
    ...
    // ✅ ESTADOS PARA TOOLTIP PERSONALIZADO
    ...
    // ✅ FUNCIÓN PARA OBTENER MENSAJE DEL TOOLTIP
    const getRowTooltip = (row) => {
        const editableStatuses = ['EN_PROCESO', 'EN_CALIDAD', 'CALIDAD_DICTAMINADA', 'TOLERANCIA_EXCEDIDA'];
        if (editableStatuses.includes(row.status)) {
            return 'Clic para editar operación';
    ...
    // ✅ EVENTOS PARA TOOLTIP PERSONALIZADO
    const handleRowMouseEnter = (row, event) => {
        const editableStatuses = ['EN_PROCESO', 'EN_CALIDAD', 'CALIDAD_DICTAMINADA', 'TOLERANCIA_EXCEDIDA'];
        if (editableStatuses.includes(row.status)) {
            const rect = event.currentTarget.getBoundingClientRect();
            setTooltipPosition({ x: rect.left + rect.width / 2, y: rect.top });
            setTooltipMessage(getRowTooltip(row));
    ...
    const columns = useMemo(() => [
        ...
        { name: 'Serie/Lote', selector: row => row.Origen_Lote.substring(0, 11), sortable: true, width: '110px' },
        { name: 'Nro. MultiOp', selector: row => row.NumeroMultiOperacion, sortable: true, width: '110px' },
        { name: 'Cortes', selector: row => row.Operacion_Cuchillas, width: '170px', style: { whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } },
        { name: 'Prog', selector: row => parseFloat(row.KilosProgramadosEntrantes || 0).toFixed(0), sortable: true, right: true, width: '80px' },
        { name: 'Stock', selector: row => parseFloat(row.Stock || 0).toFixed(0), sortable: true, right: true, width: '80px' },
        { name: 'Batch', selector: row => row.NroBatch, sortable: true, width: '90px' },
        { name: 'Balanza', selector: row => parseFloat(row.Kilos_Balanza || 0).toFixed(0), sortable: true, right: true, width: '90px' },
        ...
        { name: 'Flia.', selector: row => row.Familia, sortable: true, width: '70px', center: true },
        { name: 'Esp.', selector: row => row.Espesor, sortable: true, width: '70px', right: true },
        { name: 'Fecha Inicio', selector: row => row.batch_FechaInicio, format: row => formatCustomDate(row.batch_FechaInicio), sortable: true, width: '160px' },
        { 
            name: 'Consulta', 
            cell: row => (
                <div className="tooltip-container" style={{ position: 'relative', display: 'inline-block' }}>
                    ...
                    <span className="tooltip-text">Ver Detalle de la Operación</span>
                </div>
            ), 
            ignoreRowClick: true, 
            center: true 
        ...
        { name: 'Tarea', selector: row => row.Tarea, sortable: true, wrap: true },
        { name: 'Paq/Ata', selector: row => row.Paquetes, sortable: true, width: '90px', center: true },
        { name: 'Hoj/Roll', selector: row => row.Rollos, sortable: true, width: '100px', center: true },
    ], [navigate]);

    // ✅ ESTILOS CONDICIONALES (solo cursor, SIN font-size)
    const conditionalRowStyles = [
        {
            when: row => row.status === 'BLOQUEADA',
            style: { backgroundColor: '#dc3545', color: 'white', cursor: 'default' }
        },
        {
            when: row => row.status === 'LISTA',
            style: { backgroundColor: '#28a745', color: 'white', cursor: 'default' }
        },
        {
            when: row => row.status === 'EN_PROCESO',
            style: { backgroundColor: '#6c757d', color: 'white', cursor: 'pointer' }
        },
        {
            when: row => row.status === 'EN_CALIDAD' || row.status === 'CALIDAD_DICTAMINADA' || row.status === 'TOLERANCIA_EXCEDIDA',
            style: { backgroundColor: '#ffc107', color: 'black', cursor: 'pointer' }
        },
        {
            when: row => row.status === 'SUSPENDIDA',
            style: { backgroundColor: '#e9ecef', color: 'black', cursor: 'default' }
        }
    ];
